...
<?php
  session_start();
  $_SESSION['kode']='C';
  $_SESSION['halaman']="uknown_10";
  $_SESSION['loket']='CPU';
  $_SESSION['jenis']='CPU';

  $conn=mysqli_connect("localhost","root","","antrian_mh");
  $hasil=mysqli_query($conn,"SELECT * FROM table_no_antrian WHERE st='' AND jenis='CPU' ORDER BY id asc");
...
<body >
  <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    <div class="container-fluid">
      <a class="navbar-brand" >Loket CPU</a>
    ...
    <div class="card-body">
      ...
      <div class="card">
        <div class=" navbar-dark bg-dark">
          <center><h1 style="color:aliceblue">Loket CPU</h1></center>
        </div>
  ...
  <br>
  <?php
    $hasil=mysqli_query($conn,"SELECT * FROM table_no_antrian WHERE st='' AND jenis='CPU' ORDER BY no_antrian asc");
    $data=mysqli_fetch_array($hasil);
  ...
  <?php
  $hasil=mysqli_query($conn,"SELECT * FROM table_no_antrian WHERE st='sudah' AND jenis='CPU' ORDER BY no_antrian desc"); 
  $data=mysqli_fetch_array($hasil);
  $_SESSION['kode_C']='C';  
  ...
  ?>
    <div class="container-fluid">
      <div class="grid gap-0 row-gap-3">
        <div class="row">
          <div class="col-sm-9 col-md-6 bg-warning " id="main">
            <div id="title_main">
              <center><h1 style="color:aliceblue">No mendatang</h1></center>
            </div>
            <br>
            <strong>
              <center><h1 class="card-body" id="font" alt="..."><?php echo $_SESSION['kode_C']," ",sprintf("%02d",$no_dt_belum)  ?></h1></center>
            </strong>
          </div>
          <div class="col-sm-9 col-md-6 bg-success" id="main" >
            ...
            <br>
            <strong>
              <center><h1 class="card-body" id="font" alt="..."><?php echo $_SESSION['kode_C']," ",sprintf("%02d",$no_dt_sudah)  ?></h1></center>
            </strong>
            </div>
          </div>
        </div>
