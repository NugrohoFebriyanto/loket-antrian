...
<?php
  session_start();
  $_SESSION['kode']='P';
  $_SESSION['halaman']="uknown_8";
  $_SESSION['loket']='Printer';
  $_SESSION['jenis']='Printer';

  $conn=mysqli_connect("localhost","root","","antrian_mh");
  $hasil=mysqli_query($conn,"SELECT * FROM table_no_antrian WHERE st='' AND jenis='Printer' ORDER BY id asc");
...
<body >
  <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    <div class="container-fluid">
        <a class="navbar-brand" >Loket Printer</a>
    ...
    <div class="card-body">
      ...
      <div class="card">
        <div class=" navbar-dark bg-dark">
          <center><h1 style="color:aliceblue">Loket Printer</h1></center>
        </div>
  ...
  <br>
  <?php
    $hasil=mysqli_query($conn,"SELECT * FROM table_no_antrian WHERE st='' AND jenis='Printer' ORDER BY no_antrian asc");
    $data=mysqli_fetch_array($hasil);
  ...
  $hasil=mysqli_query($conn,"SELECT * FROM table_no_antrian WHERE st='sudah' AND jenis='Printer' ORDER BY no_antrian desc"); 
  $data=mysqli_fetch_array($hasil);
  $_SESSION['kode_P']='P';  
      ...
      <div class="grid gap-0 row-gap-3">
        <div class="row">
          <div class="col-sm-3 col-md-6 bg-warning " id="main">
            ...
            <br>
            <strong>
              <center><h1 class="card-body" id="font" alt="..."><?php echo $_SESSION['kode_P']," ",sprintf("%02d",$no_dt_belum)  ?></h1></center>
            </strong>
          </div>
          <div class="col-sm-9 col-md-6 bg-success" id="main" >
            ...
            <br>
            <strong>
              <center><h1 class="card-body" id="font" alt="..."><?php echo $_SESSION['kode_P']," ",sprintf("%02d",$no_dt_sudah)  ?></h1></center>
            </strong>
            </div>
          </div>
        </div>
